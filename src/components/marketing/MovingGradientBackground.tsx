 ...
 */
export function MovingGradientBackground({ intensity = 'soft' }: { intensity?: 'soft' | 'strong' } = {}) {
  // 'strong' = l'équivalent d'une seconde couche superposée (1-(1-o)²), sans le
  // coût d'un double rendu. Utilisé par GlassShell ; le marketing reste en 'soft'.
  const o = (soft: number, strong: number) => (intensity === 'strong' ? strong : soft)
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      ...
      <span className="kv-blob absolute -top-[10%] -left-[4%] h-[34vw] w-[34vw] rounded-full blur-[90px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-gold) 32%, white)', opacity: o(0.5, 0.75), animation: 'kvDrift1 24s ease-in-out infinite' }} />
      <span className="kv-blob absolute -top-[6%] right-[2%] h-[32vw] w-[32vw] rounded-full blur-[90px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-diamond) 32%, white)', opacity: o(0.5, 0.75), animation: 'kvDrift2 28s ease-in-out infinite' }} />
      <span className="kv-blob absolute top-[38%] -left-[6%] h-[34vw] w-[34vw] rounded-full blur-[100px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-master) 28%, white)', opacity: o(0.45, 0.7), animation: 'kvDrift3 30s ease-in-out infinite' }} />
      <span className="kv-blob absolute top-[44%] -right-[6%] h-[32vw] w-[32vw] rounded-full blur-[100px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-legend) 28%, white)', opacity: o(0.42, 0.66), animation: 'kvDrift1 26s ease-in-out infinite reverse' }} />
      <span className="kv-blob absolute -bottom-[8%] left-[26%] h-[34vw] w-[34vw] rounded-full blur-[100px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-platinum) 36%, white)', opacity: o(0.5, 0.75), animation: 'kvDrift2 34s ease-in-out infinite' }} />
      <span className="kv-blob absolute -bottom-[6%] right-[24%] h-[30vw] w-[30vw] rounded-full blur-[90px]"
        style={{ background: 'color-mix(in oklch, var(--color-league-bronze) 30%, white)', opacity: o(0.45, 0.7), animation: 'kvDrift3 31s ease-in-out infinite reverse' }} />
    </div>
  )
}
