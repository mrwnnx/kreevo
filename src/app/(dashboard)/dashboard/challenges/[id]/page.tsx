...
export default async function ChallengePage({ params, searchParams }: Props) {
  ...
  const [
    ...
    (supabase.from('submissions') as any).select('*, profiles:user_id(username, avatar_url, league)').eq('challenge_id', id).eq('is_draft', false).eq('validation_status', 'approved').neq('user_id', user.id).order('likes_count', { ascending: false }),
